...
public:
    ...
    virtual void sendPacket_v2(DynamixelPacket_v2 &aPacket, uint16_t read_size) = 0;
    virtual void receivePacket(DynamixelPacket &aPacket, uint8_t answerSize = 0) = 0;
    virtual void receivePacket_v2(DynamixelPacket_v2 &aPacket, uint16_t answerSize = 0) = 0;
    ...
    /* V2 comp added: */
    template <class T>
    inline DynamixelStatus read_v2(uint8_t aID, uint16_t aAddress, T &aData, uint8_t aStatusReturnLevel = 2);
    template <class T>
    inline DynamixelStatus write_v2(uint8_t aID, uint16_t aAddress, const T &aData, uint8_t aStatusReturnLevel = 2);

    DynamixelStatus read_v2(uint8_t aID, uint16_t aAddress, uint16_t aSize, uint8_t *aPtr, uint8_t aStatusReturnLevel = 2);
    DynamixelStatus write_v2(uint8_t aID, uint16_t aAddress, uint16_t aSize, const uint8_t *aPtr, uint8_t aStatusReturnLevel = 2);

    DynamixelStatus ping_v2(uint8_t aID);
    DynamixelStatus action_v2(uint8_t aID = BROADCAST_ID, uint8_t aStatusReturnLevel = 2);
    DynamixelStatus reset_v2(uint8_t aID, uint8_t aStatusReturnLevel = 2);
...
/* V2 comp added: */
template <class T>
DynamixelStatus DynamixelInterface::read_v2(uint8_t aID, uint16_t aAddress, T &aData, uint8_t aStatusReturnLevel)
{
    return read_v2(aID, aAddress, uint16_t(sizeof(T)), (uint8_t *)&aData, aStatusReturnLevel);
}

template <class T>
DynamixelStatus DynamixelInterface::write_v2(uint8_t aID, uint16_t aAddress, const T &aData, uint8_t aStatusReturnLevel)
{
    return write_v2(aID, aAddress, uint16_t(sizeof(T)), (const uint8_t *)&aData, aStatusReturnLevel);
}
